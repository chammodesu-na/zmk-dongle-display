...
struct battery_object {
    lv_obj_t *symbol;
    lv_obj_t *label;
    lv_obj_t *side;   // "L" / "R" (and optional "D")
...
/*
 * Which side label to show for a given line index.
 *
 * - If CONFIG_ZMK_DONGLE_DISPLAY_DONGLE_BATTERY=y:
 *     index 0 = dongle (D)
 *     index 1.. = peripherals (L/R)
 * - Else:
 *     index 0.. = peripherals (L/R)
 *
 * NOTE: Depending on your pairing / peripheral index order, L/R might appear swapped.
 *       If so, just swap "L" and "R" in this function.
 */
static const char *side_text_for_index(int i) {
#if IS_ENABLED(CONFIG_ZMK_DONGLE_DISPLAY_DONGLE_BATTERY)
    if (i == 0) {
        return "D";
    }
...
static void set_battery_symbol(lv_obj_t *widget, struct battery_state state) {
    ...
    lv_obj_t *side   = battery_objects[state.source].side;
    ...
    if (state.level > 0 || state.usb_present) {
        ...
        lv_obj_clear_flag(side, LV_OBJ_FLAG_HIDDEN);
        lv_obj_move_foreground(side);
    } else {
        lv_obj_add_flag(symbol, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(label, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(side, LV_OBJ_FLAG_HIDDEN);
    }
}
...
int zmk_widget_dongle_battery_status_init(struct zmk_widget_dongle_battery_status *widget, lv_obj_t *parent) {
    ...
    for (int i = 0; i < ZMK_SPLIT_BLE_PERIPHERAL_COUNT + SOURCE_OFFSET; i++) {
        ...
        // Right side stack: [battery icon][% label]
        lv_obj_align(image_canvas,  LV_ALIGN_TOP_RIGHT,  0,   i * 10);
        lv_obj_align(battery_label, LV_ALIGN_TOP_RIGHT, -7,   i * 10);

        // "L" / "R" label placed to the left of the percentage.
        // If it overlaps on your font/theme, tweak the X offset (e.g. -36 .. -52).
        lv_label_set_text(side_label, side_text_for_index(i));
        lv_obj_align(side_label, LV_ALIGN_TOP_RIGHT, -44, i * 10);

        lv_obj_add_flag(image_canvas,  LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(battery_label, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(side_label,    LV_OBJ_FLAG_HIDDEN);
